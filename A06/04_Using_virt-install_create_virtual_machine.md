...
1. Let's launch a Rocky 9 installation using `virt-install`:
    ```
    sudo virt-install --name=rocky-9-cli-vm --vcpus=2 --memory=3072 --location https://mirror.netzwerge.de/rocky-linux/9.3/BaseOS/x86_64/os/ --disk pool=vms,size=10,bus=virtio --os-variant=rocky9 --network network='default',model=virtio --extra-args='console=ttyS0,115200n8 serial' --nographics
    ```
